...
public class TCPPacket {
    ...
    public static boolean isClientHello(byte[] packet, int dataOffset) {
        ...
        if (dataLen < TLS_HEADER_LEN) {
            //Log.e(TAG, "Invalid TLS Packet. Less than TLS Header Length");
            // incomplete request
            return false;
        }
        ...
        if (index < packet.length) {
            ...
            if (tlsVersionMajor < TLS_VERSION_MAJOR) {
                //Log.e(TAG, "TLS Version under 1.0. Cannot support SNI");
                return false;
            }

            final int tlsDataLength = ((packet[index+3] << 8 & 0x0000FF00) | (packet[index+4] & 0x000000FF)) + TLS_HEADER_LEN;
            if (dataLen < tlsDataLength) {
                //Log.e(TAG, "Invalid TLS Packet. Data length reported " + tlsDataLength + " is more than packet data length - " + dataLen);
                return false;
            }
    ...
    public static String extractServerNameFromClientHello(byte[] packet, int dataOffset) {
        ...
        while(extensionsCounter < extensionsLength && index < packet.length) {
            ...
            if (extensionType == SERVER_NAME) {
                ...
                if (serverNameType == 0) {
                    ...
                    //Log.e(TAG, "Server extension found but no server name");
                    return serverName;
                }
            }
        ...
        //Log.e(TAG, "extractServerNameFromClientHello: no server name found");
